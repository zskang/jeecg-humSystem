...
public class CgAutoListController extends BaseController {
	...
	public void list(String id, HttpServletRequest request, HttpServletResponse response,String type) {
		...
		String html = viewEngine.parseTemplate(TemplateUtil.getTempletPath(entity, 0, TemplateUtil.TemplateType.LIST),
				paras);
	...
	public void datagrid(String configId, String page, String field, String rows, String sort, String order,
		...
		boolean isTree = configs.get(CgAutoListConstant.CONFIG_ISTREE) == null ? false
				: CgAutoListConstant.BOOL_TRUE
						.equalsIgnoreCase(configs.get(CgAutoListConstant.CONFIG_ISTREE).toString());
					...
					for (Map r : result) {
						...
						for (DictEntity dictEntity : dicDataList) {
							if (value.equalsIgnoreCase(dictEntity.getTypecode())) {
								...
								r.put(bean.getFieldName(),
										MutiLangUtil.getMutiLangInstance().getLang(dictEntity.getTypename()));
	...
	@RequestMapping(params = "toTry")
	@ResponseBody
	public AjaxJson toTry(String configId, String id, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		String jversion = cgFormFieldService.getCgFormVersionByTableName(configId);
		String table = (String) configService.queryConfigs(configId, jversion).get(CgAutoListConstant.TABLENAME);
		cgTableService.toTry(table, id);
		String message = "录用成功";
		systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
		j.setMsg(message);
		return j;
	 ...
	 * @return
	 */
	@RequestMapping(params = "toTryBatch")
	@ResponseBody
	public AjaxJson toTryBatch(String configId, String ids, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		String jversion = cgFormFieldService.getCgFormVersionByTableName(configId);
		String table = (String) configService.queryConfigs(configId, jversion).get(CgAutoListConstant.TABLENAME);
		String message = "录用成功";
		try {
			String[] id = ids.split(",");
			cgTableService.tryBatch(table, id);
		} catch (Exception e) {
			message = e.getMessage();
		}
		systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
	...
	private void loadIframeConfig(Map<String, Object> paras, HttpServletRequest request) {
		...
		if (!request.getQueryString().contains("isHref")) {
			...
			// sb.append("<link id=\"easyuiTheme\" rel=\"stylesheet\"
			// href=\"plug-in/easyui/themes/"+cssTheme+"/easyui.css\"
			// type=\"text/css\"></link>");
			...
			// sb.append("<link rel=\"stylesheet\"
			// href=\"plug-in/tools/css/common.css\"
			// type=\"text/css\"></link>");
			sb.append(SysThemesUtil.getCommonTheme(sysThemesEnum));
			// sb.append("<script type=\"text/javascript\"
			// src=\"plug-in/lhgDialog/lhgdialog.min.js\"></script>");
			sb.append(SysThemesUtil.getLhgdialogTheme(sysThemesEnum));
			sb.append(StringUtil.replace(
					"<script type=\"text/javascript\" src=\"plug-in/tools/curdtools_{0}.js\"></script>", "{0}", lang));

			sb.append("<script type=\"text/javascript\" src=\"plug-in/tools/easyuiextend.js\"></script>");
			// if("metro".equals(cssTheme)){
			// sb.append("<link id=\"easyuiTheme\" rel=\"stylesheet\"
			// href=\"plug-in/easyui/themes/"+cssTheme+"/main.css\"
			// type=\"text/css\"></link>");
			// }
	...
	private List<DictEntity> queryDic(String dicTable, String dicCode, String dicText) {
		...
		// dicSql.append(" "+"(SELECT ID FROM
		// "+CgAutoListConstant.SYS_DICGROUP+" WHERE TYPEGROUPCODE =
		// '"+dicCode+"' )");
		// }else{//step.2 如果给定了字典表则使用该字典表，这个功能需要在表单配置追加字段
		// //table表查询
		// dicSql.append("SELECT DISTINCT ").append(dicCode).append(" as
		// typecode, ");
		...
		// }
		// //step.3 字典数据
		// List<Map<String, Object>> dicDatas =
		// systemService.findForJdbc(dicSql.toString());
