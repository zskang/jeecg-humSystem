...
public class PinyinUtil {
	...
	/**
	 * 只转换第一个汉字为拼音，并且只取 第一个字母 大写。
	 * 
	 * @param src
	 * @return
	 */
	public static String getPinyinWithMark(String inputString) {
		// 汉语拼音格式输出类
		HanyuPinyinOutputFormat hanYuPinOutputFormat = new HanyuPinyinOutputFormat();
		// 输出设置，大小写，音标方式等
		hanYuPinOutputFormat.setCaseType(HanyuPinyinCaseType.UPPERCASE); // 大写
		char[] input = inputString.trim().toCharArray();
		StringBuffer output = new StringBuffer();
		try {
			String[] temp = PinyinHelper.toHanyuPinyinStringArray(input[0], hanYuPinOutputFormat);
